...
RefMother <- RawMother %>% 
  ...
  filter(!(!is.na(CONLLEGADA) & Arrival_date > ESTECLOS1)) %>% # Remove nest where chicks hatched before nest monitoring because we don't know the brood size.
...
SampleSize_Facilitation <- Facilitation %>% count(SibPresence); SampleSize_Facilitation
```

# Save sample size 
```{r}
write_csv(SampleSize_Facilitation, "SampleSize_Facilitation.csv")
```
...
# Add climatic data 
```{r}
DATABio_Clim <- Facilitation %>% arrange(NIDO) %>% 
  rowwise() %>% 
  mutate(listChl = list(Clim_imput$Chl[Clim_imput$Date %within% interval(ESTECLOS1, FECHFINAL1)])) %>%
  mutate(listSST = list(Clim_imput$SST[Clim_imput$Date %within% interval(ESTECLOS1, FECHFINAL1)])) %>% 
  mutate(listArea_rain = list(Clim_imput$Area_Rain[Clim_imput$Date %within% interval(ESTECLOS1,
                                                                                     FECHFINAL1)])) %>% 
      ungroup() %>% 
  mutate(
...
SampleSize_Facilitation <- Data_Cox %>% count(SibPresence); SampleSize_Facilitation
```
...
ClimCount_na <- ClimCount %>% select(-contains("list")) %>% 
  mutate(across(c(WORKYEAR), as.character)) %>% 
  # mutate(across(c(WORKYEAR, CoupleExpLocal, RealMotherAge), as.numeric)) %>% 
  filter(!(WORKYEAR) < 2003) %>% 
  mutate(WORKYEAR = as.character(WORKYEAR)) %>% 
  mutate(across(c(Chl, SST, Area_rain), arm::rescale))

colSums(is.na(ClimCount_na))

Nest_remove <- ClimCount_na %>% filter(Death601 == 1 & (is.na(Chl) | is.na(SST))) %>% .$NIDO

ClimCount_naomit <- ClimCount_na %>% filter(!NIDO %in% Nest_remove) %>% glimpse()
ClimCount_naomit %>% count(NIDO, SibPresence) %>% count(SibPresence)
```


## Analysis nest survival

It's not interesting because the chick that lived with a brother live by default longer. So analyze from 5 days to age of death 

Cox model
```{r}
ClimCount_naomit %<>% mutate(SibPresence = fct_relevel(SibPresence, "1", after = 3L))
levels(ClimCount$SibPresence)
...
saveRDS(Cox, file = "Cox_Facilitation.rds")
summary(Cox)
readRDS("Cox_Facilitation.rds")

Coxph <- coxph(Surv(Start_time, Stop_time, Death601) ~ SibPresence +
               PROPORTIONALRANK +
               # SibPresence : Chl + # No Sign
               SibPresence : SST +
               Chl +
...
Cox1 <- coxme(Surv(Time601, Death601)  ~ SibPresence +
               PROPORTIONALRANK +
                 # SibPresence : SST +
                 Chl +
                 I(Chl^2) +
...
Bestsurvival = glmmTMB(Fledge ~ SibPresence +
               PROPORTIONALRANK +
                 SibPresence : Chl +
                 Chl +
                 I(Chl^2) +
               ...
               (1|WORKYEAR),
                     ...
                     data = Data_Cox) #cloglog winer vs logit / z0 winner vs Z1

summary(Bestsurvival)
plot(allEffects(Bestsurvival))
```
Chl don't have a sign effect on chick A survival neither the interaction
SST don't have an interaction effect with SibPresence 
## Cox plot
```{r, eval=FALSE}
Cox <- coxph(Surv(Time601, Death601) ~ strata(SibPresence) +
               PROPORTIONALRANK +
               # Chl +
                 # I(Chl^2) +
                 SST +
             ...
             data = Data_Cox)
summary(Cox)
ggsurv <- ggsurvplot(survfit(Cox), data = Data_Cox, conf.int = T, palette = "Dark2", 
...
### Plot cox
```{r}
CoxmeWA = coxph(Surv(Time0, Time, BroodReduction) ~
                  PROPORTIONALRANK + strata(Event) +
                  HatchAsync +
                  RefittedChl +
                  I(RefittedChl^2) +
                  ...
                  frailty(NIDO),
                data = CoxData)
ggsurv <- ggsurvplot(survfit(CoxmeWA), data = CoxData, conf.int = T, palette = "Dark2", 
           censor = FALSE)
```
...
NDpred %>%
  ...
  geom_smooth(color = "#009DF0") + #method = "lm", formula = y ~ x + I(x^2), size = 1) +
  geom_hline(yintercept = 1, size = 1.1, color = "#F04610", linetype = "dashed") +
  geom_vline(xintercept = 26.8, size = 0.5, color = "#F04610", linetype = "dashed") +
  labs(y = "Hazard Ratio", x = "SST") +
  theme_gdocs() + theme(axis.title = element_text(size = 27),
